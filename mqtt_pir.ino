...
// Variabeles
    ...
    // Set the pin for PIR sensor :D7 = GPIO13    
    int pirPin = 13;

    // set the default pit state as low (no motion)
    int state = LOW;
...
void callback(char* topic, byte* payload, unsigned int length) 
    {
        ...
        if (newTopic == "YOUR CLIENT NAME/commands") 
        {
            ...
            if (newPayload == "enable-pir")
            {
            Serial.println("Enable PIR command received");
            client.publish ("checkin/YOUR CLIENT NAME","Enable PIR Command received");
...
void loop()
{
    ...
    // If the pin goes high send Motion Detected to the mqtt server
    ...
    {            
      ...
      if (state == LOW) 
      {
        ...
        client.publish ("YOUR CLIENT NAME/PIR","Motion Detected");
        state = HIGH;
      }
